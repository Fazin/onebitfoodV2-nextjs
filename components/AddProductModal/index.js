...
export default function AddProductModal(props) {
  ...
  const addProduct = (e) => {
    ...
    // else if (cart.products.map((temp) => temp.id).includes(product.id)) {
    //   var newCart = cart.products;
    // //   newCart.map((item, index) => {
    // //     if (item.id == product.id) {newCart[index].quantity += product.quantity};
    // //   });
    //   setCart({
    ...
    >
      ...
      <Modal.Body>
        ...
        <Row>
          <Col>
            <p>
              <small>{truncateString(props.product.description, 60)}</small>
            </p>
          </Col>
        </Row>
